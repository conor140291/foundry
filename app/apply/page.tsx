 ...
 * - Step 1: Create account (email → magic link)
 * - Step 2: Basic details
 * - Step 3: ID verification (photo ID + selfie upload)
 * - Step 4-6: Application questions
...
const EMPTY_FORM: FormData = {
  email: "", fullName: "", age: "", location: "", socialHandle: "",
  q1: "", q2: "", q3: "", q4: "", q5: "", q6: "",
};
  ...
  return (
    <div style={{ display: "flex", gap: 3, marginBottom: 40 }}>
      {Array.from({ length: total }).map((_, i) => (
        <div key={i} style={{
          flex: 1, height: 2,
          background: i < step ? "#10b981" : i === step ? "#555" : "#1e1e1e",
...
function StepHeader({ step, total, title, subtitle }: {
  ...
  return (
    <div style={{ marginBottom: 36 }}>
      ...
      <h2 style={{ fontSize: 28, fontWeight: 300, letterSpacing: "-0.02em", color: "#e2e0db" }}>
        {title}
      </h2>
    </div>
  );
...
function Field({
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
      ...
      {rows ? (
        <textarea
          id={name} rows={rows} placeholder={placeholder}
...
function UploadZone({
  label, icon, uploaded, uploading, error, fileName, onFileSelect,
}: {
      ...
      <div className={className} onClick={() => ref.current?.click()}>
        {uploading ? (
          <div>
            ...
            <div style={{ fontSize: 13, color: "#f59e0b" }}>Uploading securely...</div>
          </div>
        ) : uploaded ? (
          <div>
            <div style={{ fontSize: 20, marginBottom: 8 }}>✓</div>
            <div style={{ fontSize: 13, color: "#10b981" }}>{fileName}</div>
            <div style={{ fontSize: 11, color: "#555", marginTop: 4 }}>Click to replace</div>
          </div>
        ) : (
          <div>
...
export default function ApplyPage() {
  ...
  const [uploads, setUploads] = useState<UploadState>({
    idPhotoPath: null, selfiePath: null,
    idPhotoName: null, selfiePhotoName: null,
    idUploading: false, selfieUploading: false,
    idError: null, selfieError: null,
  });
  ...
  const uploadFile = async (
    file: File,
    bucket: "operator-ids" | "play-receipts",
    type: "id" | "selfie"
  ) => {
    const isId = type === "id";
    setUploads(prev => ({
      ...prev,
      [isId ? "idUploading" : "selfieUploading"]: true,
      [isId ? "idError" : "selfieError"]: null,
    }));
    ...
    try {
      ...
      setUploads(prev => ({
        ...prev,
        [isId ? "idUploading" : "selfieUploading"]: false,
        [isId ? "idError" : "selfieError"]: err.message || "Upload failed. Please try again.",
      }));
    }
  };
    ...
    try {
      const res = await fetch("/api/apply", {
        ...
        body: JSON.stringify({
          ...
          idPhotoPath:  uploads.idPhotoPath,
  ...
  const canAdvance = (): boolean => {
    switch (step) {
      ...
      case 1: return !!form.fullName && !!form.age && !!form.location;
    ...
    return (
      <div style={{ minHeight: "100vh", background: "#090909", display: "flex", alignItems: "center", justifyContent: "center", padding: 40 }}>
        <div style={{ maxWidth: 520, textAlign: "center" }}>
          ...
          <h1 style={{ fontSize: 36, fontWeight: 300, letterSpacing: "-0.02em", color: "#e2e0db", marginBottom: 16 }}>
            Application received.
          </h1>
          <p style={{ color: "#666", lineHeight: 1.9, fontSize: 14, marginBottom: 40 }}>
            Every application is reviewed manually, usually within 48 hours.
            Strong operators may receive an immediate trial allocation. We'll be in touch.
          </p>
          <div className="card" style={{ padding: 24, textAlign: "left" }}>
            <div className="label" style={{ marginBottom: 16 }}>What happens next</div>
            {[
              "ID verification reviewed",
              "Application scored by team",
              "Decision emailed within 48h",
              "Approved operators receive first allocation",
            ].map((item, i) => (
              <div key={i} style={{
                display: "flex", gap: 14, padding: "11px 0",
                borderBottom: i < 3 ? "1px solid #1a1a1a" : "none",
                fontSize: 13, color: "#888", alignItems: "center",
              }}>
                <span className="mono" style={{ color: "#333", minWidth: 24, fontSize: 11 }}>
    ...
    <>
      ...
      <nav style={{
        ...
        <a href="/" style={{ textDecoration: "none" }}>
          <span className="mono" style={{ fontSize: 15, fontWeight: 500, letterSpacing: "0.2em", color: "#e2e0db" }}>
            FOUNDRY
          </span>
        </a>
      </nav>
      ...
      <div style={{ minHeight: "100vh", paddingTop: 100, paddingBottom: 80, display: "flex", justifyContent: "center" }}>
        <div style={{ width: "100%", maxWidth: 640, padding: "0 24px" }}>
          ...
          <div style={{ marginBottom: 48 }}>
            <div className="label" style={{ marginBottom: 12 }}>Capital application</div>
            <h1 style={{ fontSize: 42, fontWeight: 300, letterSpacing: "-0.02em", color: "#e2e0db", marginBottom: 8 }}>
              Apply for Foundry
            </h1>
            <p style={{ color: "#555", fontSize: 14, lineHeight: 1.7 }}>
              Reviewed manually. We look for resourcefulness, judgment, and execution ability.
              No CV required.
            </p>
          </div>
          ...
          {step === 0 && (
            <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
              <Field
                ...
                hint="We'll send a magic link to verify your email. No password needed."
              />
              {!magicLinkSent ? (
                <button
                ...
                <div className="card" style={{ padding: 20, borderColor: "#1a3a1a" }}>
                  ...
                  <div style={{ fontSize: 13, color: "#666", lineHeight: 1.7 }}>
                    Check your inbox at <strong style={{ color: "#888" }}>{form.email}</strong>.
                    Click the link to verify, then return here to continue.
                  </div>
                  <button
                    style={{ background: "none", border: "none", color: "#555", fontSize: 12, marginTop: 12, padding: 0, cursor: "pointer" }}
                    onClick={() => { setMagicLinkSent(false); setForm(p => ({ ...p, email: "" })); }}
                  >
          ...
          {step === 1 && (
            <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
              ...
              <Field
                ...
                hint="Optional but helpful — gives us context on your background."
              />
            </div>
          )}
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
              <div className="card" style={{ padding: 20, borderColor: "#1a3a1a" }}>
                <div style={{ fontSize: 13, color: "#888", lineHeight: 1.9 }}>
                  We verify every operator before allocating capital.
                  Your documents are stored in an encrypted private bucket
                  and are never shared publicly or with third parties.
                  See our <a href="/privacy" style={{ color: "#10b981" }}>Privacy Policy</a>.
                </div>
              </div>
              <UploadZone
                label="Photo ID — passport or driving licence"
                icon="🪪"
                uploaded={!!uploads.idPhotoPath}
                uploading={uploads.idUploading}
                error={uploads.idError}
                fileName={uploads.idPhotoName}
                onFileSelect={file => uploadFile(file, "operator-ids", "id")}
              />
              <UploadZone
                label="Selfie holding your ID — face and ID must both be clearly visible"
                icon="🤳"
                uploaded={!!uploads.selfiePath}
                uploading={uploads.selfieUploading}
                error={uploads.selfieError}
                fileName={uploads.selfiePhotoName}
                onFileSelect={file => uploadFile(file, "operator-ids", "selfie")}
              />
            </div>
          )}
          ...
          {step === 3 && (
            <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
              ...
              />
              <Field
                label="What inefficiency in the world do most people ignore?"
                name="q2" value={form.q2} onChange={setField} rows={5}
                placeholder="Something you've noticed that others haven't — in markets, in local areas, in online platforms..."
              />
            </div>
          )}
          ...
          {step === 4 && (
            <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
              <Field
                label="Tell us about a time you created value from almost nothing."
                name="q3" value={form.q3} onChange={setField} rows={6}
                placeholder="A deal, a side hustle, a fix, a trade. Anything real. Be specific about the numbers."
              />
              <Field
                label="What kinds of opportunities are you naturally best at spotting?"
                name="q4" value={form.q4} onChange={setField} rows={5}
                placeholder="Arbitrage? Digital services? Local markets? Specific platforms? Be honest about where your instincts are strongest."
              />
            </div>
          )}
          ...
          {step === 5 && (
            <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
              <Field
                label="If we allocated you €10,000 tomorrow, how would you deploy it?"
                name="q5" value={form.q5} onChange={setField} rows={6}
                placeholder="Be concrete. What's the exact play? What would you buy, where, from whom, and how would you exit?"
              />
              <Field
                label="What's your unfair advantage?"
                name="q6" value={form.q6} onChange={setField} rows={5}
                placeholder="Access to certain markets? An existing platform following? Specialist knowledge? A network? Be direct."
                hint="This is the question most people answer too vaguely. The best answers name something specific."
              />
              <div className="card" style={{ padding: 16, borderColor: "#222" }}>
                <div style={{ fontSize: 12, color: "#555", lineHeight: 1.8 }}>
                  By submitting, you confirm all information is accurate and you agree to Foundry's
                  operator terms including the principal return obligation.
                  View full <a href="/terms" style={{ color: "#888" }}>Operator Terms</a>.
                </div>
              </div>
            </div>
          )}
